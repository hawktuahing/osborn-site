...
### Phone specifics
The scroll meter is fixed at `z-index:300`, above the menu sheet's 95, so an open menu would otherwise have the rail sitting across the nav items. It translates off the left edge while `html.menu-open` is set, on the sheet's own 380ms curve so the two read as one gesture.
...
The counter is also rate-capped so a full 0→100 takes at least ~900ms. On a quick connection the clip lands in a few hundred milliseconds, so the number used to jump in two steps and then the screen sat still while the decoder woke up — which reads as a lag rather than as loading. The cap is not a fake (the number still never passes what has arrived); it only stops the count outrunning the eye, and it means the decode happens while the number is still climbing. On a slow connection the download is the binding constraint and the cap never applies.
...
The clip is **never handed to the element as a `blob:` URL**. The preloader reads the response body to completion purely to count bytes — that is what puts the file in the HTTP cache — and then plays it by its ordinary URL, which the element serves from that cache with nothing over the network (verified: three range requests, zero transferred bytes). An earlier version passed the downloaded bytes in as a Blob, and on iOS that made the clip play about a fifth of a second and then restart, because Safari's media stack re-prepares blob sources mid-playback. `assets/*` is given a one-hour `Cache-Control` in `vercel.json` so that hand-off is a cache hit rather than a revalidation.

`diag.html` exists to tell those cases apart on a real device: it runs the same download-then-play sequence either way, logs every media event with timings on screen, and flags any backward jump in `currentTime`. It is not linked from the site.

**The clip plays at rate 1 from the first frame — no `playbackRate` ease-in.** It used to start at 0.35 and ramp to 1 over 700ms, and bisecting on a real iPhone put the fault there: the picture snapped back to the opening frame about 60ms after the last write to `playbackRate`, in every run, while `currentTime` carried on and not one media event fired. Writing `playbackRate` makes iOS re-present the video, and re-presenting it showed frame zero. The poster, the blob source, the full-bleed box with its height transition and the backdrop-filter buttons over the clip were each ruled out first — the reset survived every one of them being switched off. The clip already opens on a slow push-in, so nothing is lost.

**The poster is removed the moment playback starts.** The poster is the clip's own first frame, and iOS re-displays a poster whenever it has to rebuild the element's rendering surface — which looks exactly like the intro starting, playing a fifth of a second and restarting, while `currentTime` never goes backwards because the clip never actually rewound. On-device testing confirmed the shape of it: a bare video with no poster played clean, the same sequence inside the hero did not, and the back-jump counter was zero in both. For the same reason `heroFit()` is a no-op while the clip is playing — a phone's toolbar sliding away fires `resize` mid-intro, and resizing a playing video is one of the things that makes iOS rebuild that surface.

Playback must not touch `currentTime`. Assigning it — even to 0, on an element already at 0 — fires a real seek; on desktop that lands within milliseconds and is invisible, but on iOS it lands around 0.4s *after* playback has begun and throws the clip back to the start, so the intro visibly played a fifth of a second and then restarted. The element is freshly loaded, so there is nothing to rewind. For related reasons `preload` flips from `none` to `auto` when the source is handed over (otherwise `load()` only takes metadata and `play()` starts a second load), readiness waits for `canplay` rather than `loadeddata`, and the ease-in writes `playbackRate` eight times over 700ms instead of on every animation frame — each write makes the media pipeline re-sync.
