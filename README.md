...
Media is picked per breakpoint and each side downloads only its own: the section photos use `<picture>` with a `media` source, and the hero `<video>` carries the desktop clip in markup (so a no-JS load still has one) with a small inline script swapping in the portrait cut before preload starts. A phone never fetches the 21MB 4K file and a desktop never fetches the portrait set — verified by checking the resource list on both.
